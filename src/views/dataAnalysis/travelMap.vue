<template>
    <div class="travel-map">
         <el-card>
            <div id="travelView" class="box"></div>
        </el-card>
    </div>
</template>
<script>
import geoJson from "@/assets/data.json";
import { gettravelData } from "@/api/index.js";
export default {
    data() {
        return {
    ...
    created() {
        gettravelData().then((res) => {
        if (res.status === 200) {
            let { points, linesData } = res.data;
  ...
  methods: {
      draw(points, linesData) {
          let myChart = this.$echarts.init(document.getElementById('travelView'))
